...
object SettradeRepository {
    ...
    suspend fun fetchLiveSetIndex(): FetchResult = withContext(Dispatchers.IO) {
        val primary = tryFetch(SET_OR_TH_URL) { html -> parseSetOrTh(html) }
        if (primary is FetchResult.Success) return@withContext primary

        val fallback = tryFetch(SETTRADE_URL) { html -> parseSettrade(html) }
        if (fallback is FetchResult.Success) return@withContext fallback
